...
class _RecipeScreenState extends State<RecipeScreen> {
  ...
  // 모드 이름 정규화 함수 (oven_pages.dart의 모드 목록과 매칭)
  ...
  // 스마트 파싱 로직 (Fake AI) - 정규표현식으로 조리법에서 오븐 설정 추출
  // 오븐/전자레인지 등 기능 옆에 온도와 함께 나오는 시간만 추출
  OvenSettings? _parseOvenSettings(List<String> steps) {
    // 조리법을 한 문장으로 합침
    String fullText = steps.join(" ");

    // 모드 패턴 (전자레인지, 오븐, 에어프라이, 해동, 스팀전자레인지, 에어수비드 등)
    // 에어프라이어와 에어프라이 둘 다 인식, 스팀전자레인지와 스팀 전자레인지 둘 다 인식
    final modeReg = RegExp(r'(전자레인지|오븐|에어프라이어?|해동|스팀\s*전자레인지|에어수비드|광파오븐)');
    ...
    for (final modeMatch in modeMatches) {
      ...
      // 모드 주변 100자 이내에서 온도와 시간을 찾기
      final searchStart = (modeStart - 20).clamp(0, fullText.length);
      final searchEnd = (modeStart + 100).clamp(0, fullText.length);
      final contextText = fullText.substring(searchStart, searchEnd);

      // 온도 패턴: 숫자 + 도/℃ (모드 뒤에 나오는 것, 선택적)
      final tempReg = RegExp(r'(\d{1,3})(도|℃)');
      final tempMatch = tempReg.firstMatch(contextText);
      ...
      // 모드와 시간이 있으면 설정 객체 생성 (온도는 선택적)
      if (timeStr != null) {
        // 모드 이름 정규화 (oven_pages.dart의 모드 목록과 매칭)
        String normalizedMode = _normalizeMode(mode);

        return OvenSettings(
          mode: normalizedMode,
          temperature: tempMatch?.group(0) ?? '0도', // 온도가 없으면 기본값
          time: timeStr,
        );
      }
    }
